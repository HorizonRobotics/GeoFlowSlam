...
int main(int argc, char *argv[]) {
  ...
  std::string setting = string(argv[2]);
...
// Read image from file
      ...
      if (im.size() != img_size) {
        ...
        //     std::chrono::steady_clock::now();
        // double dt =
        //     std::chrono::duration_cast<std::chrono::duration<double> >(tb -
        //     ta)
        //         .count() *
        //     1e6;
      }
      ...
      double ttrack =
          std::chrono::duration_cast<std::chrono::duration<double> >(t2 - t1)
              .count();
      // cout << "track cost time per frame: " << ttrack0 * 1e6 << " "
      //      << ttrack * 1e6 << " ms" << endl;
      ttrack_tot += ttrack;
      ...
      if (ttrack < T) usleep((T - ttrack) * 1e6 * nDelay);  // 1e6
    }
    if (seq < num_seq - 1) {
    ...
    //      << endl;
    // cout << "mean tracking time: " << (totaltime / vTimesTrack.size()) <<
    // endl;
  }
  ...
  double mean_time = total_time / nImages[0];
  ...
  f.close();
  // SLAM.SaveTrackTimeCost(TimeStamp_file.c_str(), vTimesTrack);
  // Save camera trajectory
