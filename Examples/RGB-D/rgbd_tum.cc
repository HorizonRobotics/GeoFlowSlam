...
int main(int argc, char **argv) {
...
#ifdef ENABLE_ASYNC
    vTimesTrack = SLAM.GetTrackTimes();
    if(!vTimesTrack.empty()) ttrack = vTimesTrack.back();
#else
  ...
  cout << "tracking lost count: " << float(SLAM.GetTrackLostCnt()) / vTimesTrack.size()
       << endl;
  // double total_time =
  ...
  // // cout << "median tracking time: " << vTimesTrack[nImages / 2] * 1e3 <<
  // "ms"
  // //      << endl;
  ...
  f << "tracking lost count: " << float(SLAM.GetTrackLostCnt()) / vTimesTrack.size()
       << endl;
  f.close();
  SLAM.SaveTrajectoryTUM2(CameraTrajectory_file.c_str());
  SLAM.SaveKeyFrameTrajectoryTUM2(KeyFrameTrajectory_file.c_str());
