...
int main(int argc, char** argv) {
  ...
  // 63 cfg.enable_stream(RS2_STREAM_GYRO, RS2_FORMAT_MOTION_XYZ32F); //, 400);
  ...
  auto imu_callback = [&](const rs2::frame& frame) {
    ...
    if (rs2::frameset fs = frame.as<rs2::frameset>()) {
      ...
      imCV = cv::Mat(cv::Size(width_img, height_img), CV_8UC3,
      (void*)(color_frame.get_data()), cv::Mat::AUTO_STEP); depthCV =
